...
import { normalizeModelConfig } from '../lib/modelConfig';
import { appendAuditLog } from '../lib/audit';
import { createRateLimiter } from '../lib/riskControl';
import { verifyActiveUserToken } from '../lib/userAuth';
import { recordModelMetric } from '../lib/modelMetrics';
import { createModelTask, getModelTask, updateModelTask } from '../lib/modelTasks';

const router = Router();
const JWT_SECRET = process.env.JWT_SECRET || 'dev-secret';
const modelGenerateLimiter = createRateLimiter({
...
type ModelError = Error & { code?: string; retryable?: boolean; status?: number };

function makeModelError(message: string, code: string, retryable = false, status?: number): ModelError {
  const err = new Error(message) as ModelError;
  err.code = code;
...
async function withOneRetry(task: () => Promise<string>) {
  try {
    return await task();
  } catch (error: any) {
    const err = error as ModelError;
    if (!err?.retryable) throw err;
    return await task();
  }
}

async function callOpenAICompatible(prompt: string, modelCfg: any) {
...
async function callAzureOpenAI(prompt: string, modelCfg: any) {
  ...
  if (!apiKey) throw makeModelError('missing apiKey', 'MODEL_CONFIG_ERROR', false, 500);
  if (!baseURL) throw makeModelError('missing baseURL', 'MODEL_CONFIG_ERROR', false, 500);
  if (!deployment) throw makeModelError('missing deployment', 'MODEL_CONFIG_ERROR', false, 500);
...
function writeModelAudit(req: any, payload: {
  action: string;
  success: boolean;
  message?: string;
  docId?: string;
...
function getSelectableModels(modelCfg: any) {
  const list = Array.isArray(modelCfg?.selectableModels) ? modelCfg.selectableModels : [];
  return list
    .filter((item: any) => item && typeof item.id === 'string' && item.id.trim())
    .map((item: any) => ({
...
function buildRuntimeModelConfig(modelCfg: any, modelChoice: string) {
  const choices = getSelectableModels(modelCfg);
  if (!choices.length) {
    const pickedId = String(modelChoice || '').trim();
    if (!pickedId || pickedId === 'default') return { runtime: modelCfg, chosen: pickedId };
    return { runtime: null, chosen: pickedId };
  }
  const pickedId = String(modelChoice || modelCfg.defaultModelChoice || choices[0].id || '').trim();
  if (!pickedId) return { runtime: modelCfg, chosen: '' };
  const hit = choices.find((item: any) => item.id === pickedId);
  if (!hit) return { runtime: null, chosen: pickedId };
  const runtime = {
    ...modelCfg,
...
async function executeGenerate(req: any, input: { prompt: string; docId?: string; modelChoice?: string }) {
  ...
  const runtimeCfg = normalizeModelConfig(runtimeModel.runtime);
  ...
  if (mode === 'disabled') {
    ...
    recordModelMetric({ durationMs: Date.now() - startedAt, success: false, mode, apiName, errorCode: 'MODEL_DISABLED' });
  ...
  if (mode === 'mock') {
    ...
    recordModelMetric({ durationMs: Date.now() - startedAt, success: true, mode, apiName });
  ...
  try {
    ...
    if (mode === 'openai_compatible') {
      ...
      recordModelMetric({ durationMs: Date.now() - startedAt, success: false, mode, apiName, errorCode: 'MODEL_UNSUPPORTED_MODE' });
    ...
    recordModelMetric({ durationMs: Date.now() - startedAt, success: false, mode, apiName, errorCode });
...
router.post('/generate/task', authMiddleware, modelGenerateLimiter, (req: any, res) => {
  const { prompt, docId, modelChoice } = req.body || {};
  ...
  setTimeout(async () => {
    updateModelTask(task.id, { status: 'running', startedAt: new Date().toISOString() });
    try {
      const result = await executeGenerate(workerReq, { prompt, docId, modelChoice });
      updateModelTask(task.id, {
        status: 'success',
        text: result.text,
...
router.get('/generate/task/:id', authMiddleware, (req: any, res) => {
  const id = String(req.params.id || '');
  if (!id) return res.status(400).json({ error: 'missing task id' });
  const task = getModelTask(id);
  if (!task) return res.status(404).json({ error: 'task not found' });
  if (task.userId !== String(req.user?.sub || '')) return res.status(403).json({ error: 'forbidden' });
