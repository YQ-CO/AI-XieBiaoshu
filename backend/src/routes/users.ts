...
import { getEnterpriseRole, loadEnterprises } from '../lib/storage';
...
router.get('/me', authMiddleware, (req, res) => {
  const user = req.user;
  // 从企业数据中查找所属企业
  const all = loadEnterprises();
  const myOrgs = all.filter(e => e.members.includes(user.sub)).map(e => ({ id: e.id, name: e.name, inviteCode: e.inviteCode, role: getEnterpriseRole(e, user.sub) }));
...
router.post('/switch', authMiddleware, (req, res) => {
  ...
  res.json({ success: true, enterprise: { id: ent.id, name: ent.name, creditCode: ent.creditCode, role: getEnterpriseRole(ent, req.user.sub) } });
});
