...
function normalizeSelectableModels(list: any): any[] {
...
export function normalizeModelConfig(modelCfg: any = {}) {
...
export function validateModelConfig(input: any): { valid: boolean; message?: string; normalized?: any } {
  ...
  for (const item of selectableModels) {
    const id = String(item?.id || '').trim();
    if (!id) return { valid: false, message: 'selectable model id is required' };
    if (seen.has(id)) return { valid: false, message: `duplicate selectable model id: ${id}` };
    seen.add(id);
  }
